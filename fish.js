  ...
  let m = new THREE.MeshLambertMaterial({
    ...
    onBeforeCompile: shader => {
      ...
      `.replace(
        ...
        col = mix(vec3(0.0, 0.2, 0.1), vec3(0.0, 0.5, 0.2), weed) * 0.85;
        ...
        gl_FragColor.rgb += vec3(caustic) * 0.25;
          ...
          gl_FragColor.rgb = mix(vec3(0, 100, 120) / 255., gl_FragColor.rgb, smoothstep(0.5, 0., circleDist));
        `
      );
      //console.log(shader.fragmentShader);
    }
...
function createFishGeometry(){
  ...
  let topPoints = topCurve.getSpacedPoints(120);
  // bottom
...
function createBubble() {
    ...
    const bubbleMaterial = new THREE.MeshBasicMaterial({ color: 0xE6E6FF, transparent: true, opacity: 0.5 });
...
function animate() {
    ...
    bubbleGroup.children.forEach(bubble => {
        bubble.position.y += 0.02; // Adjust the speed of the upward movement
        if (bubble.position.y > 10) {
            bubble.position.y = -10; // Reset the position when the bubble reaches the top
        }
    });
